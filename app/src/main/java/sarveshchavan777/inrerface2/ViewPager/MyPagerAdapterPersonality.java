...
import sarveshchavan777.inrerface2.Tab.Personality.perEasyTab;
import sarveshchavan777.inrerface2.Tab.Personality.perHardTab;
import sarveshchavan777.inrerface2.Tab.Personality.perMediumTab;


public class MyPagerAdapterPersonality extends FragmentStatePagerAdapter {
    Context context;
   ...
   private int numbOfTabs; // Store the number of tabs, this will also be passed when the ViewPagerAdapter is created
     ...
     public MyPagerAdapterPersonality(FragmentManager fm, int micon[], int mNumOfTabs, Context context) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position == 0) // if the position is 0 we are returning the First p_easyy.xml
        {
            return new perEasyTab();

        }
        else if (position==1)         // As we are having 2 tabs if the position is now 0 it must be 1 so we are returning second p_easyy.xml
        {
            return new perMediumTab();

        }
        else {
            return new perHardTab();
